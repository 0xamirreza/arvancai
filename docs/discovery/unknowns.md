...
| Drive / Logs / dedicated VPC OpenAPI | Not on FA portal product grid | No MCP tools |
| WAF/DDoS path FA vs SDK | FA `/waf`,`/ddos` vs SDK `/waf/settings` | Tools follow FA product paths |
| Authorization header prefix variants | Samples disagree | Env key sent as-is |
| CDN webhook event catalog | UNKNOWN | No webhook tools |

## Scope deferrals

- Full CDN OpenAPI surface (156 paths) — subset already in MCP
